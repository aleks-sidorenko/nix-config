...
with lib.${namespace}; rec {
  ...
  flakeDir = config:
    toString (
      if (config == null) || (config == {}) || (config ? home-manager)  
      then /. + "$HOME/.${namespace}"  # Use /. + to convert string to path
      else /. + "/home/${config.${namespace}.user.name}/.${namespace}"
    );
}
